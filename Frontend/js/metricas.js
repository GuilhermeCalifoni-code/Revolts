// Frontend/js/metricas.js (Versão Final Simplificada e Robusta)
const $ = (sel) => document.querySelector(sel);
async function fetchMetricasGerais() { const response = await fetch('/api/metricas-gerais'); if (!response.ok) { throw new Error("Falha ao buscar métricas gerais"); } return await response.json(); }
let g1, g2, g3;
function chartLine(ctx, labels, data) { return new Chart(ctx, {type:'line',data:{labels,datasets:[{label:'kW',data,borderWidth:2,fill:false,tension:0.35,borderColor:'rgba(34,197,94,0.9)',pointRadius:0}]},options:baseOptions('kW')}); }
function chartBar(ctx, labels, data) { return new Chart(ctx, {type:'bar',data:{labels,datasets:[{label:'kWh',data,backgroundColor:'rgba(59,130,246,0.85)',borderRadius:6}]},options:baseOptions('kWh')}); }
function chartDonut(ctx, labels, data) { return new Chart(ctx, { type: 'doughnut', data: { labels, datasets: [{ data, backgroundColor: ['#3B82F6','#22C55E','#F59E0B','#94A3B8','#6366F1'] }]}, options: { ...baseOptions(), cutout: '62%', plugins: { legend: { position: 'bottom', labels: { color: getTextColor() }}, tooltip: { callbacks: { label: function(context) { let label = context.label || ''; if (label) { label += ': '; } if (context.parsed !== null) { const total = context.chart.data.datasets[0].data.reduce((a, b) => a + b, 0); const percentage = total > 0 ? (context.parsed / total * 100).toFixed(1) : 0; label += `${context.raw.toFixed(2)} kWh (${percentage}%)`; } return label; } } } } } }); }
